...
\tikzset{
  kuni/.style={draw=mainBlue,line width=0.7pt,fill=white,
    minimum width=42mm,minimum height=8.5mm,inner sep=1.5pt},
  kunihead/.style={fill=mainBlue,text=white,font=\gtfamily\small,
    minimum width=42mm,minimum height=6.5mm,inner sep=1.5pt},
...
\begin{tikzpicture}[x=1mm,y=1mm]
  ...
  % 海の下地
  \fill[seaBG] (-2,0) rectangle (74,92);
  % 地域塗り分け（大陸輪郭でクリップ）
  \begin{scope}
    \clip \africapath;
  ...
  \node[font=\small\gtfamily,align=center] at (37,20)
    {中・南部\\[-1pt]{\footnotesize （英・仏・ベルギー等）}};
  % ---- 右側：分割を促した要因 ----
...
\begin{tikzpicture}[x=1mm,y=1mm]
  ...
  % 日本：福建（東から）
  \draw[shinshutsu] (106,42) -- (90,28)
    node[at start,above,font=\footnotesize\gtfamily,align=center]
      {日本：福建{\scriptsize （台湾の対岸）}};
  % イギリス：長江流域（南西から）
  \draw[shinshutsu] (8,4) -- (44,26)
    node[at start,left,font=\footnotesize\gtfamily,align=right]
      {イギリス：\\長江流域};
  % フランス：華南（南から）
  ...
  \node[draw=mainBlue,line width=0.7pt,fill=white,rounded corners=1.5mm,
        inner sep=3mm,text width=38mm,align=left,anchor=north west]
        (hannou) at (122,58) {%
...
\begin{tikzpicture}[x=0.72mm,y=0.72mm]
  ...
  \draw[shinshutsu] (106,44) -- (90,28)
    node[at start,above,font=\scriptsize\gtfamily] {日本：福建};
  \draw[shinshutsu] (8,4) -- (44,26)
    node[at start,left,font=\scriptsize\gtfamily,align=right]
      {イギリス：\\長江流域};
  \draw[shinshutsu] (74,0) -- (66,16)
